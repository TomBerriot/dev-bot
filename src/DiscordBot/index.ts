...
class DiscordBot {
    ...
    public async setup() {
        ...
        const folderPath = `${appRootPath}/src/DiscordBot/Commands`;
        ...
        for (const file of commandFiles) {
            ...
            if (command.nonce)
                this.bot.reactionsHandlers.set(command.nonce, command);
        }
    ...
    public message(message) {
        try {
            if (
                message.content.substring(0, discordBotConfig.prefix.length) ===
                discordBotConfig.prefix
            ) {
                const args = message.content
                    .substring(discordBotConfig.prefix.length)
                    .split(/ +/);
                ...
                logger.info(
                    `${message.content} : author : ${
                        ...
                        message.guild ? message.guild.name : 'no guild '
                    }`
                );
                ...
                if (command.guildOnly && message.channel.type !== 'text') {
                    return message.channel.send(
                        'This command is reserved to guilds.'
                    );
                } else if (
                    command.ownerGuildOnly &&
                    (message.member.id !== message.member.guild.ownerID ||
                        message.author.tag !== 'Onodera#3602')
                ) {
                    return message.reply(
                        'You must be the owner of the guild to use this role'
                    );
                }
    ...
    public messageReactionAdd(reaction, user) {
        try {
            if (user.id !== this.bot.user.id) {
                const command = this.bot.reactionsHandlers.get(
                    reaction.message.nonce
                );
